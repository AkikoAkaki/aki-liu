(function () {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (archiveItems.length > 0) {
            ...
            const isFilterableArchive = archiveList && !document.body.classList.contains('section-microblog');
            if (isFilterableArchive) {
                const tagControls = [...document.querySelectorAll('.archive-tag-item[data-tag]')];
                const yearControls = [...document.querySelectorAll('.archive-year-item[data-year]')];
                const entryStat = document.querySelector('[data-archive-stat="entries"]');
                const yearStat = document.querySelector('[data-archive-stat="years"]');
                const reduceMotionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
                const selectedTags = new Set();
                const selectedYears = new Set();
                const validTags = new Set(tagControls.map(el => normalizeToken(el.dataset.tag)).filter(Boolean));
                const validYears = new Set(yearControls.map(el => normalizeYear(el.dataset.year)).filter(Boolean));
                ...
                function readFiltersFromURL(useInitialTags) {
                    ...
                    const tags = urlTags.length || params.has('tags') || !useInitialTags ? urlTags : initialTags;
                ...
                function matchesFilters(item) {
                    ...
                    const matchesYear = selectedYears.size === 0 || selectedYears.has(year);
                ...
                function updateFilterControls() {
                    tagControls.forEach(el => {
                        el.classList.toggle('active', selectedTags.has(normalizeToken(el.dataset.tag)));
                    });
                    yearControls.forEach(el => {
                        el.classList.toggle('active', selectedYears.has(normalizeYear(el.dataset.year)));
                    });
                }
                ...
                function finishFilterTransition(token, beforeRects, toShow, matchedSet) {
                ...
                function applyFilters(options = {}) {
                    ...
                    const animate = options.animate !== false && !reduceMotionQuery.matches;
                ...
                const hadURLTags = new URLSearchParams(window.location.search).has('tags');
